...
// in one transaction.
func (s *Store) CreateSecret(id, vaultID, key string, nonce, ciphertext []byte, versionID, createdBy string) (*model.Secret, error) {
	ts := now()
	tx, err := s.db.Begin()
	if err != nil {
		return nil, err
	}
	_, err = tx.Exec(`INSERT INTO secrets (id, vault_id, key, nonce, ciphertext, version, created_by, created_at, updated_at)
		VALUES (?, ?, ?, ?, ?, 1, ?, ?, ?)`,
		id, vaultID, key, nonce, ciphertext, createdBy, ts, ts)
	if err != nil {
		_ = tx.Rollback()
	...
	return &model.Secret{ID: id, VaultID: vaultID, Key: key, Version: 1,
...
func (s *Store) GetSecretByID(id string) (*model.Secret, []byte, []byte, error) {
	row := s.db.QueryRow(`SELECT id, vault_id, key, nonce, ciphertext, version, created_by, created_at, updated_at
...
func (s *Store) ListSecretsByVault(vaultID string) ([]*model.Secret, error) {
	rows, err := s.db.Query(`SELECT id, vault_id, key, version, created_by, created_at, updated_at
	...
	for rows.Next() {
		...
		var c, u string
		if err := rows.Scan(&sec.ID, &sec.VaultID, &sec.Key, &sec.Version, &sec.CreatedBy, &c, &u); err != nil {
			return nil, err
		}
...
func scanSecret(row *sql.Row) (*model.Secret, []byte, []byte, error) {
	...
	var nonce, ct []byte
	var c, u string
	err := row.Scan(&sec.ID, &sec.VaultID, &sec.Key, &nonce, &ct, &sec.Version, &sec.CreatedBy, &c, &u)
	if err != nil {
		if errors.Is(err, sql.ErrNoRows) {
